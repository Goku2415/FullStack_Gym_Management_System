...
const MemberDetail = () => {
  ...
  return (
    <div className="w-3/4  text-violet-600 p-2">
       <div className="p-4 rounded-lg w-full text-xl text-white bg-slate-500 border-2 shadow-2xl">
          ...
          <span onClick={()=>{navigate(-1)}} className="border-2 rounded-2xl hover:bg-[#1e3d7b] mx-auto my-auto cursor-pointer p-2"> <ArrowBackIcon sx={{ cursor: "pointer", marginBottom: "2px" }} />Go Back</span> 
        ...
        <div className="mt-4 bg-white p-4 h-[80%] rounded-lg shadow-2xl">
          <div className="w-full h-fit flex ">
            ...
            <div className="w-2/3 text-xl p-2">
              ...
              <div onClick={()=>{setRenew(prev => !prev)}}  className={`mt-1 rounded-lg p-3 border-2 border-slate-800 ${renew && status==='Active'?'bg-linear-to-r from-indigo-600 via-purple-500 not-only:to-pink-500' :null}  w-40 md:w-1/2 cursor-pointer hover:text-white hover:bg-linear-to-r from-indigo-500 via-purple-500 to-pink-500 `}>Renew

            </div>

            {
              renew && status==='active' ? ( <div className="rounded-lg p-3 mt-5  mb-5 h-fit bg-slate-200 w-[80%] ">
